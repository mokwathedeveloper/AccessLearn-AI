...
export class RegistryService {
  ...
  private async syncUsers() {
    ...
    for (const authUser of users) {
      ...
      if (!profile) {
        this.logger.warn(`[SYNC] Missing profile for user ${authUser.id}, creating...`);
        ...
        else this.logger.error(`Failed to repair user ${authUser.id}`, insertError.message);
      }
    }
  ...
  private async syncMaterials() {
    ...
    // This is a complex one, we'll check if files in DB actually exist in Storage
    ...
    for (const material of materials as any[]) {
      const { data: fileExists } = await this.supabase.storage
        ...
        .list(material.file_url.split('/')[0], {
          search: material.file_url.split('/')[1],
        });
      ...
      if (!fileExists || fileExists.length === 0) {
        ...
        if (material.status !== 'failed') {
          await this.supabase
            .from('materials')
            .update({ status: 'failed', description: (material.description || '') + ' [System: File Missing]' })
            .eq('id', material.id);
          missingFiles++;
        }
      }
    ...
    return { status: 'success', checked: materials.length, missingFiles };
  }

  private async cleanupStuckMaterials() {
    this.logger.log('[SYNC] Cleaning up stuck processing materials...');
    // Materials stuck in 'processing' for more than 30 minutes
    ...
    if (error) return { status: 'error', message: error.message };
    return { status: 'success', cleaned: data?.length || 0 };
  }
}
